...
//
// begin
void LiquidCrystal_I2C::begin(uint8_t sda, uint8_t scl, uint8_t cols, uint8_t lines, uint8_t dotsize)
{
   
   init(sda, scl);     // Initialise the I2C expander interface
...
//
// init
int LiquidCrystal_I2C::init(uint8_t sda, uint8_t scl)
{
   int status = 0;
   ...
   // ------------------------------------------------------------------------
   if ( _i2cio.begin (sda, scl,  _Addr ) == 1 )
   {
